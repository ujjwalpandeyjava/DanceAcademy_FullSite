...
public class AuthController {
	...
	public AuthController(AuthenticationManager authenticationManager, JWTHelper jwtHelper, UserDetailsService userDetailsService, PasswordEncoder passwordEncoderBCrypt,
			EmailDetails eDetails, EmailSendingService emailService, UserRegistrationRepo userRegistrationRepo, RegisteredUserRepo registeredUserRepo) {
	 ...
	 * User are created by administration, account get verified when the user verifies with the OTP the account get registered for the user...
	 * 
	 * @param userReg
	 * @return
	 */
	...
	public ResponseEntity<Map<String, String>> sendOTPForRegistration(@Valid @RequestBody UserRegistration userReg) {
		...
		if(alreadyRegisteredUser.isPresent())
			return ResponseEntity.status(HttpStatus.ALREADY_REPORTED).body(Map.of(Constant.MESSAGE, Constant.USER_EXISTS));
		else {
			...
			if (mailSent == true) {
				...
				if (byEmailID.isPresent()) {
					...
					userReg.setOtp(otpString);
					userReg.setExpireDateTime(Instant.now().plus(Duration.ofMinutes(Constant.ONE_TWENTY)).toString());
				} 
				savedDetails = userRegistrationRepo.save(userReg);
				return ResponseEntity.status(HttpStatus.ACCEPTED).body(Map.of(Constant.MESSAGE, Constant.SUCCESS,
						Constant.Action, "Check provided email for link and OTP for verification. Valid for " + Constant.ONE_TWENTY + " minutes!",
						"UserID", savedDetails.getId()));
			} else
				return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of(Constant.MESSAGE, Constant.INCORRECT_EMAIL));
		}
	}
		...
		if (verifyingUser.isPresent()) {	// Change to the registered user...
			...
			if (verUser.getOtp().equals(userReg.getOtp())) {
				...
				String encodedPass = passwordEncoderBCrypt.encode(verUser.getDefaultPassword());
				 RegisteredUser newUserDetails = new RegisteredUser(verUser.getEmailID(), encodedPass, true, true, true, true, List.of(new SimpleGrantedAuthority(verUser.getAuthorities())));
				...
				return ResponseEntity.status(HttpStatus.CREATED).body(Map.of(Constant.MESSAGE, Constant.SUCCESS));
			} else
				return ResponseEntity.status(HttpStatus.NOT_ACCEPTABLE).body(Map.of(Constant.MESSAGE, "Incorrect OTP"));
		} else 
			return ResponseEntity.status(HttpStatus.UNAVAILABLE_FOR_LEGAL_REASONS).body(Map.of(Constant.MESSAGE, "Record with id '" + userReg.getId() + "' not found!"));
	}
	...
	public ResponseEntity<Map<String, Object>> loginAndGetToken(@Valid @RequestBody LoginReqBody body) {
		...
		if(!passwordEncoderBCrypt.matches(body.getPassword(), userDetail.getPassword())) 
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of(Constant.MESSAGE, "Incorrect password"));
...
//	        UserDetails userDetail2 = (UserDetails) authentication.getPrincipal();
    ...
    public ResponseEntity<Map<String, String>> logout() {
        ...
        return ResponseEntity.ok(Map.of(Constant.MESSAGE, "Token revoked successfully"));
    }
	...
	public ResponseEntity<Map<String, Object>> handleValidationExceptions(Exception ex) {
		...
		if (ex instanceof MethodArgumentNotValidException) {
			...
			errors.put(Constant.MESSAGE, ex1.getMessage());
		} else if (ex instanceof HttpMessageNotReadableException) {
			var ex1 = (HttpMessageNotReadableException) ex;
			System.err.println(ex1.getMessage());
			errors.put(Constant.MESSAGE, "Request Body missing!");
			...
			errors.put(Constant.MESSAGE, "User " + ex1.getMessage() + " not found!");
		} else {
